...
SCRATCH_OUTFOLDER="$SCRATCH/doctorate/uniaxial-deformation/3d-phantom/$OUTFOLDER"
LAMMP_IN_FOLDER="input"
INPUT_FILE_TO_DIRECTION="input_raw/uniaxial_fast_tension_3d_phantom.in"
mkdir -p $OUTFOLDER
...
for CHAINFILE in structure/*.structure.out; do
  ...
  for TEMP in 300; do
    for ERATE in 2 6 10; do
      for DIR_COMBI in xyz yzx zxy; do
        ...
        INPUT_DIR3="${DIR_COMBI:2:2}"

        echo "$INPUT_DIR1 $INPUT_DIR2 $INPUT_DIR3 $ERATE $TEMP"
