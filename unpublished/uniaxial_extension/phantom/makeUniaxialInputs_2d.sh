...
INPUT_FILE_TO_DIRECTION="input_raw/uniaxial_tension_2d_phantom.in"
mkdir -p $OUTFOLDER
...
for CHAINFILE in structure/*.out; do
  ...
  for TEMP in 0 150 300 450; do
    for ERATE in 2 4 10; do
      for DIR_COMBI in xy yx; do
        ...
        sed -i "s/INPUT_ERATE/$ERATE""e-5/gi" "$TARGET_INPUT"
