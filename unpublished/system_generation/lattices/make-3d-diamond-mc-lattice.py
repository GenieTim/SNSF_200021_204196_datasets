...
nrOfCellsInDirX = 10  # int(1.5*36) # 4
nrOfCellsInDirY = 10  # 36 # 2
nrOfCellsInDirZ = 10
    ...
    for xcell in range(int(nrOfCellsInDirX)):
        ...
        for zcell in range(int(nrOfCellsInDirZ)):
            ...
            crosslinkerCoordinates.extend([
                (currentX, currentY, currentZ+b),
                (currentX+c, currentY, currentZ+c)
            ])
            # central ("private") atoms
            crosslinkerCoordinates.extend([
                (currentX+0.5*c, currentY+0.5*c, currentZ+b-0.5*c),
                (currentX+b-0.5*c, currentY+0.5*c, currentZ+0.5*c),
                (currentX+0.5*c, currentY+b-0.5*c, currentZ+0.5*c),
                (currentX+b-0.5*c, currentY+b-0.5*c, currentZ+b-0.5*c)
            ])
            # remaining two in-duplicate faces
            crosslinkerCoordinates.extend([
                (currentX+c, currentY+c, currentZ),
                (currentX+b, currentY+c, currentZ+c)
            ])
            # next it
            currentZ += b
        currentX += b
...
# for other purposes
availableSites = np.zeros(len(crosslinkerCoordinates)) + 4
...
fig.savefig(os.path.join(os.path.dirname(__file__),
            "figure-mc-3d-diamond-lattice-{}{}-{}x{}.png".format("no-boundary-" if not plotBoundary else "", chainLength, nrOfCellsInDirX, nrOfCellsInDirY)), bbox_inches="tight", dpi=300)

# verify the functionality of each atom
for i in range(len(crosslinkerCoordinates)):
    if (crosslinkerFunctionality[i] != 4):
        warnings.warn("Found crosslinker {} with functionality {} != 4".format(
