...
# input
monomersPerChain = 24
numberOfChains = 48 #int(1e2) # 1e4
monomerDistance = 1.0
seed = 8804 + monomersPerChain + numberOfChains
...
for i in range(numberOfChains):
    ...
    chain = getCppSAWLC(monomersPerChain-1, monomerDistance, 0.75)
    assert(len(chain) == monomersPerChain)
...
print("LAMMPS FENE chain data file: {} chains with {} beads per chain and a momomer distance of {}".format(
    numberOfChains, monomersPerChain, monomerDistance))
print("""
          {} atoms
          {} bonds
          0  angles
          ...
          0  dihedral types
          0  improper types
""".format(len(chainsDf), numberOfChains*(monomersPerChain-1)))
print("""
      {}       {} xlo xhi
      {}       {} ylo yhi
      {}       {} zlo zhi
""".format(minima["x"], maxima["x"], minima["y"], maxima["y"], -0.5, 0.5))
print("""
Masses
...
for index, atom in chainsDf.iterrows():
    print("         {}       {}       {}    {}    {}    {}   0   0   0".format(
        atomIdx, moleculeIdx, atomType, atom[0], atom[1], 0))
    atomIdx += 1
    if (mode_swappable):
        if (moleculeIdx == monomersPerChain/2 and mIdxCountDirection == "up"):
        ...
        if (mIdxCountDirection == "up"):
            ...
            moleculeIdx -= 1
    if ((atomIdx-1) % monomersPerChain == 0):
        if (mode_swappable):
            moleculeIdx = 1
            mIdxCountDirection = "up"
        else:
...
bondIdx = 1
for i in range(1, monomersPerChain*numberOfChains, 1):
    # skip certain
    if (bondIdx % monomersPerChain != 0):
        print("        {}  1        {}        {}".format(
